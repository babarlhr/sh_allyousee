...
class WooInstanceConfig(models.TransientModel):
    ...
    _description = "Woo Res Config Instance"
    ...
    def woo_test_connection(self):
        ...
        if not isinstance(r, requests.models.Response):
            raise UserError(_("Response is not in proper format :: %s" % (r)))
        if r.status_code != 200:
            raise UserError(_("%s\n%s" % (r.status_code, r.reason)))

        if self.is_export_update_images:
            """Checking if username and password are correct or not."""
            client = base.Client('%s/xmlrpc.php' % (host), self.woo_admin_username,
                                 self.woo_admin_password, transport=SpecialTransport())
            try:
        ...
        instance = instance_obj.create({'name':self.name,
    ...
    def test_and_reset_woo_credentials(self):
        ...
        instance = woo_instance_obj.browse(self._context.get('woo_instance_id'))
        wp_api = False if self.woo_version == 'v3' else True
        wcapi = woocommerce.api.API(url=instance.woo_host, consumer_key=self.woo_consumer_key,
                                    consumer_secret=self.woo_consumer_secret,
                                    verify_ssl=instance.woo_verify_ssl,
                                    wp_api=wp_api,
                                    version=instance.woo_version, query_string_auth=True)
        ...
        if self.is_export_update_images:
            """Checking if username and password are correct or not."""
            client = base.Client('%s/xmlrpc.php' % (instance.woo_host), self.woo_admin_username,
                                 self.woo_admin_password, transport=SpecialTransport())
            try:
                client.call(media.UploadFile(""))
            except InvalidCredentialsError as error:
    ...
    def action_open_woo_instance_wizard(self):
        ...
        action = ir_action_obj._for_xml_id(
                "woo_commerce_ept.res_config_action_woo_instance")
...
class ResConfigSettings(models.TransientModel):
    ...
    def _get_woo_default_financial_statuses(self):
        if self._context.get('default_woo_instance_id', False):
            financial_status_ids = self.env['woo.sale.auto.workflow.configuration'].search(
                    [('woo_instance_id', '=',
                      self._context.get('default_woo_instance_id', False))]).ids
    ...
    def execute(self):
        ...
        if instance:
            ...
            values[
                'woo_payment_term_id'] = self.woo_payment_term_id and self.woo_payment_term_id.id or False
    ...
    def action_woo_open_basic_configuration_wizard(self):
        ...
        try:
            view_id = self.env.ref(
                'woo_commerce_ept.woo_basic_configurations_onboarding_wizard_view')
        except:
            return True
    ...
    def woo_save_financial_status_configurations(self):
        ...
        if instance:
            ...
            unlink_for_financials_status and unlink_for_financials_status.unlink()
        return True
