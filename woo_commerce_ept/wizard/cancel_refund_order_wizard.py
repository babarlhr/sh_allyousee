...
class woo_cancel_order_wizard(models.TransientModel):
    ...
    def cancel_in_woo(self):
        ...
        _logger.info("Cancel sale order: %s in Woocommere store with Woo commerce order id: %s"%(order.name,order.woo_order_id))
        if not isinstance(response, requests.models.Response):
            raise UserError("Cancel Order \nResponse is not in proper format :: %s" % response)

        if response.status_code in [200, 201]:
            order.write({'canceled_in_woo': True, 'woo_status':'cancelled'})
        else:
            raise UserError("Error in Cancel Order %s" % response.content)
    ...
    def woo_create_credit_note(self, order_id):
        ...
        Task Id: 167148
        """
        moves = order_id.invoice_ids.filtered(lambda m: m.move_type == 'out_invoice' and
